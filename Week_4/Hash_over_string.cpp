...
int calculateHashCode(string s, int m) {
    int hashcode = 0;
    int k = s.size();
    
    for(int i = 0; i < k; i++) {
        hashcode = (hashcode * 256 + s[i]) % m; // shift the hashcode left by 8 bits to effectively make room for the next s[i]
    }
    return hashcode;
}
...
int main() {
    ...
    for(int i = 1; i <= n; i++) {
        cin >> s;
        res.push_back(calculateHashCode(s, m));
    }
